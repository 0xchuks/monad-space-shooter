  ...
  const loop = useCallback(
    (ts: number) => {
      ...
      if (keysRef.current['ArrowLeft'] || keysRef.current['a']) {
        p.x = Math.max(PLAYER_W / 2, p.x - PLAYER_SPEED);
      }
      if (keysRef.current['ArrowRight'] || keysRef.current['d']) {
        p.x = Math.min(W - PLAYER_W / 2, p.x + PLAYER_SPEED);
      }
      ...
      for (const a of asteroidsRef.current) {
        ...
        // Fell off screen
        if (a.y - a.r > H) {
          // just remove it (optional: could end game here)
        }
      }
        ...
        >
          <button
            ...
            style={{
              marginTop: 8,
              padding: '10px 28px',
              background: isConnected ? '#0e76fd' : '#333',
            ...
            onClick={() => {
              if (isConnected) {
                alert(`Submitting score ${finalScore} to leaderboard… (not yet wired up)`);
              }
            }}
          >
            {isConnected ? 'Submit to leaderboard' : 'Connect wallet to submit'}
          </button>
        </div>
      )}
    </div>
